...
export default function Chat() {
    ...
    // Update progress based on user message count
    useEffect(() => {
        const newProgress = Math.min((userMessageCount / 15) * 100, 100);
    ...
    return (
        <div className="min-h-screen bg-[#0a0a0a] flex">
            {/* Left Sidebar with Big Emoji */}
            <div className="w-64 h-screen bg-[#121418] border-r border-white/10 flex flex-col items-center justify-center fixed left-0 top-0">
                <div className="text-9xl transition-transform duration-300 hover:scale-110 flex items-center justify-center">
                    {currentEmoji}
                </div>
            </div>
            ...
            <div className="flex-1 flex flex-col ml-64">
                {/* Green Progress Line - Always Visible */}
                <div className="h-1 bg-green-500/20 relative overflow-hidden">
                    <div
                        className="h-full bg-green-500 absolute top-0 transition-all duration-500 ease-out"
                        style={{
                            width: `${progress}%`,
                            animation: progress < 100 ? 'slide 2s ease-in-out infinite' : 'none'
                        }}
                    ></div>
                </div>
                ...
                <header className="bg-[#1a1a1a] border-b border-white/10 px-4 py-3 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-white font-semibold text-lg">Interview Practice</h1>
